                                        ...
                                        const int align_center)
{
  ...
  // Two lineages place the anchor grid differently and the decoder must match
  // whichever generated the training targets, or every box lands with a
  // systematic offset (0.08 m mean on the KITTI grid):
  //   align_center = 1  bin CENTRES, min + (i + 0.5) * span / n
  //                     -- this repo's PyTorch model (Anchors.get_anchors)
  //   align_center = 0  ENDPOINT to endpoint, min + i * span / (n - 1)
  //                     -- OpenPCDet's default, and NVIDIA's shipped model
  float x_offset, y_offset;
  if (align_center) {
    x_offset = min_x_range + (col + 0.5f) * (max_x_range - min_x_range) / feature_x_size;
    y_offset = min_y_range + (row + 0.5f) * (max_y_range - min_y_range) / feature_y_size;
  } else {
    x_offset = min_x_range + col * (max_x_range - min_x_range) / (feature_x_size - 1);
    y_offset = min_y_range + row * (max_y_range - min_y_range) / (feature_y_size - 1);
  }
  ...
  // The reference decode (pointpillars.py::get_predicted_bboxes_single) ranks
  // ANCHORS by their max class score, keeps the top nms_pre of them, and then
  // emits a box for EVERY class whose own score clears the threshold -- not
  // just the argmax class. Emitting argmax-only dropped ~6% of the reference's
  // detections; having no nms_pre cap at all more than doubled the box count.
  //
  // The cap needs a global ranking, which a per-anchor thread cannot do, so the
  // kernel emits every qualifying (anchor, class) pair tagged with its anchor
  // index and PointPillar::doinfer applies the cap on the host.
...

  {
    ...
    // decode once -- the box regression head is class-agnostic, so every class
    // emitted for this anchor shares the same geometry
    float xa = anchor[0];
    float ya = anchor[1];
    float za = anchor[2];
    float dxa = anchor[3];
    ...
    const float anchor_index = static_cast<float>(loc_index * num_anchors + ith_anchor);

    for (int i = 0; i < num_classes; i++) {
      float cls_score = sigmoid(scores[i]);
      if (cls_score <= score_thresh) {
        continue;
      }
      int resCount = (int)atomicAdd(object_counter, 1);
      if (resCount >= max_boxes) {          // never observed; ~350 of 321408 pass at 0.1
        atomicSub(object_counter, 1);
        break;
      }
      float *data = bndbox_output + resCount * kBndboxStride;
