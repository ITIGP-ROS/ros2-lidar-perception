        ...
        unsigned int *voxel_idxs)
{
  ...
  // Bound check. Without this the kernel writes past voxel_num / voxel_idxs /
  // voxel_features whenever a cloud yields more non-empty pillars than the
  // buffers hold -- a genuine out-of-bounds write, and the reason a dense
  // KITTI cloud faulted here. Just as importantly, pillar_num is fed to the
  // network as `voxel_num`: if it exceeds the engine's voxel tensor extent the
  // scatter plugin reads uninitialised memory and the detections become noise.
  // Undoing the increment keeps the reported count at exactly min(found, cap).
  if (current_pillarId >= MAX_VOXELS) {
    atomicSub(pillar_num, 1);
    return;
  }
    ...
    float* features)
{
    ...
    //store output
    if (point_idx < pointsNumSM[pillar_idx_inBlock]) {
#if PILLAR_FEATURE_LAYOUT_MMDET3D9
      // mmdet3d-style 9 channels, as this repo's PyTorch PillarEncoder builds
      // them. Note channels 0 and 1 hold the pillar-centre offsets rather than
      // the raw x/y ("in consistent with mmdet3d" in the Python), and are then
      // repeated at 7 and 8. Every value here is one the 10-channel path
      // already computes -- this is a permutation, not different maths.
      pillarOutSM[pillar_idx_inBlock][point_idx][0] = center.x;
      pillarOutSM[pillar_idx_inBlock][point_idx][1] = center.y;
...
#else
      // OpenPCDet PillarVFE with USE_ABSLOTE_XYZ: raw xyzi, cluster offsets,
      // pillar-centre offsets.
      ...
      // Zero the padded points. Bounded by FEATURES_SIZE rather than written
      // out longhand, so the 9- and 10-channel layouts share it and neither can
      // write past the end.
      #pragma unroll
      for (int f = 0; f < FEATURES_SIZE; ++f) {
        pillarOutSM[pillar_idx_inBlock][point_idx][f] = 0;
      }
    }
