...
{
    // Suppression is PER CLASS, matching the reference decode, which runs its
    // NMS inside a `for i in range(nclasses)` loop. Suppressing across classes
    // lets a high-scoring Car delete an overlapping Pedestrian, which the
    ...
    for (size_t i = 0; i < bndboxes.size(); i++) {
        ...
        for (size_t j = i + 1; j < bndboxes.size(); j++) {
            if (suppressed[j] == 1 || bndboxes[j].id != bndboxes[i].id) {
                continue;
            }
...
int PostProcessCuda::doPostprocessCuda(const float *cls_input, float *box_input, const float *dir_cls_input,
                                        float *bndbox_output, int max_boxes, int *num_boxes_out)
{
  ...
  checkCudaErrors(postprocess_launch(cls_input,
                     ...
                     params_.dir_offset,
                     max_boxes,
                     params_.anchor_align_center ? 1 : 0,
                     stream_
                     ));
  // Read the count from the atomic counter. The upstream code parked it in
  // bndbox_output[0], where every emitting thread overwrote it -- last writer
  // won, not the largest, so the host could see fewer boxes than were written.
  checkCudaErrors(cudaMemcpyAsync(num_boxes_out, object_counter_, sizeof(int),
                                  cudaMemcpyDeviceToHost, stream_));
  checkCudaErrors(cudaStreamSynchronize(stream_));
  if (*num_boxes_out > max_boxes) {
    *num_boxes_out = max_boxes;
  }
  return 0;
}
