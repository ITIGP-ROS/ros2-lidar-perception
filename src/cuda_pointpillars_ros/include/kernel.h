...
const int FEATURES_SIZE = PILLAR_FEATURES;   // set by params.h from the model config
...
// Floats per decoded box in the postprocess output buffer:
//   x, y, z, w, l, h, yaw, class_id, score, anchor_index
// anchor_index lets the host reproduce the reference's nms_pre cap, which ranks
// ANCHORS (not boxes) by their max class score. See postprocess_kernels.cu.
const int kBndboxStride = 10;
