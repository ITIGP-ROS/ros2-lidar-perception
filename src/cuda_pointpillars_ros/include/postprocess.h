...
class PostProcessCuda {
  ...
  public:
    ...
    // Writes at most max_boxes records of kBndboxStride floats to bndbox_output
    // and reports how many it wrote in *num_boxes_out.
    int doPostprocessCuda(const float *cls_input, float *box_input, const float *dir_cls_input,
                          float *bndbox_output, int max_boxes, int *num_boxes_out);
};

// Greedy NMS, suppressing only within a class (Bndbox::id), as the reference does.
