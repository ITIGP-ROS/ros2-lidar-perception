# Adapted from xinshuoweng/AB3DMOT (MIT License), AB3DMOT_libs/dist_metrics.py
# https://github.com/xinshuoweng/AB3DMOT
# Distance / affinity metrics for the LIDAR frame (z up). The boxes live in the
# x-y ground plane, so all IoU computations are done in BEV (x-y).
import numpy as np
...
def compute_inter_2D(boxa_bottom, boxb_bottom):
    # compute intersection area of two sets of bottom corner points
    _, I_2D = convex_hull_intersection(boxa_bottom, boxb_bottom)
    return I_2D


def compute_height(box_a, box_b, inter=True):
    # overlap height along z (lidar, z up); box z is the bottom center

    corners1 = Box3D.box2corners3d(box_a)   # 8 x 3
    corners2 = Box3D.box2corners3d(box_b)   # 8 x 3
...
def compute_bottom(box_a, box_b):
    # obtain ground corners (z = 0 plane) in CCW order

    corners1 = Box3D.box2corners3d(box_a)   # 8 x 3
    corners2 = Box3D.box2corners3d(box_b)   # 8 x 3

    # get bottom corners (indices 4..7) and reverse to counter-clockwise order
    boxa_bot = corners1[7:3:-1, [0, 1]]     # 4 x 2
    boxb_bot = corners2[7:3:-1, [0, 1]]     # 4 x 2
...
def convex_area(boxa_bottom, boxb_bottom):
    # compute the convex area of the union of the two boxes (BEV)
...
def iou(box_a, box_b, metric='giou_3d'):
    """Compute 3D/2D bounding box IoU, only working for object parallel to ground (lidar frame, z up).
    ...
    if '2d' in metric:            # return 2D IoU/GIoU
        ...
        assert False, 'metric %s is not supported' % metric
...
def dist3d(bbox1, bbox2):
    ...
    corners1 = Box3D.box2corners3d(bbox1)   # 8 x 3
    corners2 = Box3D.box2corners3d(bbox2)   # 8 x 3
